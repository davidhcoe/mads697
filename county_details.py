...
from utilities import get_dataframe, get_metric, get_parameter, get_wiki_image, FIPS_PARAMETER
...
def show_county_details_page():
    ...
    if len(fips_code) == 0 or fips_code.isnumeric()==False:
        ...
        if len(county_only_df) == 1:
            ...
            if wiki_image != 0:
                st.markdown('<style>.container {        display: flex;        align-items: center;        justify-content: center}      img {        max-width: 100%}      .image {        flex-basis: 40%}      .text {        font-size: 14px;        padding-left: 20px;}</style>', unsafe_allow_html=True)
                st.markdown(f'<div class="container"><div class="image"><img src="{wiki_image}" alt="Wikipedia image of {name}"/></div><div class="text"><h1>{name}</h1><a href="#population">Population</a> | <a href="#strong-and-healthy-families">Strong and Healthy Families</a> | <a href="#supportive-communities">Supportive Communities</a> | <a href="#opportunities-to-learn-and-earn">Opportunities to Learn and Earn</a></div>', unsafe_allow_html=True)
